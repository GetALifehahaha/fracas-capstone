...
def critical_message(barangay_name: str, score: float) -> tuple[str, str]:
    ...
    body = (
        ...
        f"(score {score:.0f}/100). Prepare to evacuate and monitor local advisories. — FRACAS"
    )
    return title, body
...
def all_clear_message(barangay_name: str, level: str) -> tuple[str, str]:
    ...
    body = f"Flood risk in {barangay_name} has dropped to {level.upper()}. Stay alert. — FRACAS"
    return title, body
